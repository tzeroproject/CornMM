...
export const videoService = {
  async getVideos(options:VideoFilterOptions = {}):Promise<{videos:Video[];total:number}> { const p = new URLSearchParams(); const status = options.includeUnpublished || options.includeAllStatusForUser ? undefined : (options.status || 'published'); const visibility = options.includeUnpublished || options.includeAllStatusForUser ? undefined : (options.visibility || 'public'); if(options.categoryId)p.set('categoryId',options.categoryId); if(options.creatorId)p.set('creatorId',options.creatorId); if(options.searchQuery)p.set('searchQuery',options.searchQuery); if(options.sortBy)p.set('sortBy',options.sortBy); if(status)p.set('status',status); if(visibility)p.set('visibility',visibility); if(options.page)p.set('page',String(options.page)); p.set('pageSize',String(options.pageSize || 12)); return api(`/api/videos?${p.toString()}`); },
