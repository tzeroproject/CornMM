...
export const VideoPlayer:React.FC<VideoPlayerProps>=({video,onProgress,onComplete})=>{
 ...
 const isFilemoonEmbed=provider==='filemoon'||/filemoon\.org\/[^/]+\/embed/i.test(url);
 const isUpload18Embed=provider==='upload18'||/upload18\.(?:net|org)\/play\//i.test(url)||/upload18\.(?:net|org)\/e\//i.test(url);
 const isExternal=isUqloadEmbed||isDoodstreamEmbed||isFilemoonEmbed||isUpload18Embed;
 ...
 useEffect(()=>{
  ...
  const onError=()=>fallback(); el.addEventListener('error',onError);
  if(!url)fallback();
  ...
  el.defaultMuted=false;el.muted=false;el.volume=.9;
  return()=>{disposed=true;el.removeEventListener('error',onError);if(hls)hls.destroy()}
 },[video.id,video.video_url,video.duration,url]);
 ...
 const togglePlay=()=>{const el=videoRef.current;if(!el)return;if(el.paused){if(!isMuted){el.muted=false;el.volume=volume>0?volume:.9}el.play().then(()=>{setIsPlaying(true);setHasStarted(true)}).catch(()=>{});}else{el.pause();setIsPlaying(false)}};
 ...
 if(isExternal||useBunnyIframeEmbed){
  let src=isExternal?url:getBunnyIframeUrl({videoId:video.bunny_video_id!,autoplay:hasStarted});
  const m=src?.match(/src\s*=\s*["'](.*?)["']/i);if(m?.[1])src=m[1];
  return <div className="relative aspect-video w-full rounded-2xl overflow-hidden bg-black shadow-2xl border border-white/10"><iframe src={hasStarted?src:undefined} title={video.title} loading="lazy" className="w-full h-full border-0" allow="autoplay; fullscreen; picture-in-picture" referrerPolicy="strict-origin-when-cross-origin"/><button aria-label="Play video" onClick={()=>setHasStarted(true)} className={`absolute inset-0 z-10 flex items-center justify-center overflow-hidden ${hasStarted?'hidden':''}`}><img src={video.thumbnail_url} alt="" loading="eager" decoding="async" className="absolute inset-0 w-full h-full object-cover blur-2xl scale-110"/><span className="absolute inset-0 bg-black/40"/><span className="z-20 w-16 h-16 rounded-full bg-white/10 border border-white/20 text-white flex items-center justify-center"><Play className="w-8 h-8 fill-current ml-1"/></span></button></div>
 }
 return <div ref={containerRef} className="group relative aspect-video w-full rounded-2xl overflow-hidden bg-black shadow-2xl border border-white/10 select-none"><video ref={videoRef} poster={video.thumbnail_url} preload="metadata" onClick={togglePlay} onTimeUpdate={handleTimeUpdate} onEnded={()=>{setIsPlaying(false);onComplete?.()}} onLoadedMetadata={()=>{const el=videoRef.current;if(el){el.muted=isMuted;el.volume=isMuted?0:(volume||.9);setDuration(el.duration||video.duration||0)}}} playsInline controlsList="nodownload noplaybackrate" className={`w-full h-full object-contain cursor-pointer ${hasStarted?'':'blur-2xl scale-105'}`}/>{!hasStarted&&<button aria-label="Play video" onClick={togglePlay} className="absolute inset-0 z-10 flex items-center justify-center bg-black/30"><span className="w-16 h-16 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white flex items-center justify-center"><Play className="w-7 h-7 fill-current"/></span></button>}<div className="absolute top-4 left-4 z-20 opacity-0 group-hover:opacity-100 focus-within:opacity-100"><button onClick={()=>setUseBunnyIframeEmbed(true)} className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#111]/90 text-[11px] text-amber-300 border border-amber-500/30"><Sparkles className="w-3.5 h-3.5"/> Bunny Stream Embed</button></div><div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/95 via-black/75 to-transparent p-3 sm:p-4 flex flex-col gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity"><input aria-label="Video progress" type="range" min={0} max={duration||100} step={.1} value={Math.min(currentTime,duration||100)} onChange={handleSeek} className="w-full h-1.5 appearance-none bg-white/20 accent-amber-400 cursor-pointer"/><div className="flex items-center justify-between text-white text-xs"><div className="flex items-center gap-2 sm:gap-3"><button aria-label={isPlaying?'Pause':'Play'} onClick={togglePlay} className="p-1.5 rounded-lg hover:bg-white/10">{isPlaying?<Pause className="w-5 h-5 fill-current"/>:<Play className="w-5 h-5 fill-current"/>}</button><button aria-label="Rewind 10 seconds" onClick={()=>seekRelative(-10)} className="p-1.5 rounded-lg hover:bg-white/10"><RotateCcw className="w-4 h-4"/></button><button aria-label={isMuted?'Unmute':'Mute'} onClick={toggleMute} className="p-1.5 rounded-lg hover:bg-white/10">{isMuted||volume===0?<VolumeX className="w-4 h-4"/>:<Volume2 className="w-4 h-4"/>}</button><span className="font-mono text-[11px]">{formatTime(currentTime)} / {formatTime(duration)}</span></div><div className="flex items-center gap-1"><div className="relative"><button aria-label="Playback speed" onClick={()=>setShowSpeedMenu(v=>!v)} className="p-1.5 rounded-lg hover:bg-white/10"><Settings className="w-4 h-4"/></button>{showSpeedMenu&&<div className="absolute bottom-9 right-0 p-1 rounded-xl bg-[#111] border border-white/10 shadow-xl flex gap-1">{[.75,1,1.25,1.5,2].map(s=><button key={s} onClick={()=>setSpeed(s)} className={`px-2 py-1 rounded text-[10px] ${playbackSpeed===s?'bg-amber-500 text-black':'text-white hover:bg-white/10'}`}>{s}x</button>)}</div>}</div><button aria-label={isFullscreen?'Exit fullscreen':'Fullscreen'} onClick={toggleFullscreen} className="p-1.5 rounded-lg hover:bg-white/10">{isFullscreen?<Minimize className="w-5 h-5"/>:<Maximize className="w-5 h-5"/>}</button></div></div></div></div>;
};
