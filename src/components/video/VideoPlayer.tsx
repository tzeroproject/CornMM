...
export const VideoPlayer: React.FC<VideoPlayerProps> = ({ video, onProgress, onComplete }) => {
  ...
  const isExternal = isUqloadEmbed || isDoodstreamEmbed || isFilemoonEmbed;
  ...
  useEffect(() => {
    ...
    if (!url) fallback();
      ...
      if (Hls.isSupported()) {
        ...
        hls.loadSource(url);
        hls.attachMedia(el);
        hls.on(Hls.Events.ERROR, (_e, d) => { if (d.fatal) fallback(); });
  ...
  const togglePlay = () => {
    ...
    if (!el) return;
    if (el.paused) {
      if (!isMuted) { el.muted = false; el.volume = volume > 0 ? volume : 0.9; }
  ...
  const handleTimeUpdate = () => {
    const el = videoRef.current;
    if (!el) return;
    const curr = el.currentTime, dur = el.duration || video.duration || 0;
    setCurrentTime(curr); setDuration(dur);
    if (onProgress && (curr - lastProgressRef.current >= 5 || curr < 1)) { lastProgressRef.current = curr; onProgress(curr, dur); }
    if (!viewRecordedRef.current && curr >= 5) { viewRecordedRef.current = true; videoService.recordView(video.id); }
  };
  ...
  const toggleMute = () => { const el = videoRef.current; if (!el) return; const m = !isMuted; setIsMuted(m); el.muted = m; if (!m) { const v = volume || 0.9; el.volume = v; setVolume(v); } };
  ...
  if (isExternal || useBunnyIframeEmbed) {
    let src = '';
    if (isFilemoonEmbed) {
      // Always use the canonical FileMoon embed URL. Some API responses include /en/.
      src = providerId ? `https://filemoon.org/${encodeURIComponent(providerId)}/embed` : url.replace(/https?:\/\/filemoon\.org\/en\//i, 'https://filemoon.org/');
      if (!/\/embed(?:[/?#]|$)/i.test(src)) src = src.replace(/\/?$/, '/embed');
    } else {
      src = isExternal ? url : getBunnyIframeUrl({ videoId: video.bunny_video_id!, autoplay: hasStarted });
      const m = src?.match(/src\s*=\s*["'](.*?)["']/i);
      if (m?.[1]) src = m[1];
    }

    // Do not place a transparent/custom Play button over external providers.
    // FileMoon's own player controls must receive the click directly.
    return <div ref={containerRef} className="relative aspect-video w-full rounded-2xl overflow-hidden bg-black shadow-2xl border border-white/10">
      <iframe src={src || undefined} title={video.title} loading="eager" className="w-full h-full border-0" allow="autoplay; fullscreen; picture-in-picture; encrypted-media" allowFullScreen referrerPolicy="no-referrer-when-downgrade" />
    </div>;
  }
