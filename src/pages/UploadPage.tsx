...
type UploadMode = 'bunny' | 'uqload' | 'upload18' | 'cloudinary' | 'embed';
...
export default function UploadPage() {
  ...
  const uploadToCloudinary = async (file: File) => {
    ...
    if (!cloudName || !uploadPreset) throw new Error('Cloudinary is not configured. Set VITE_CLOUDINARY_CLOUD_NAME and VITE_CLOUDINARY_UPLOAD_PRESET.');
    const formData = new FormData();
    formData.append('file', file);
    formData.append('upload_preset', uploadPreset);
    if (title.trim()) formData.append('context', `title=${title.trim()}`);
    const response = await new Promise<any>((resolve, reject) => {
      const xhr = new XMLHttpRequest();
      xhr.open('POST', `https://api.cloudinary.com/v1_1/${encodeURIComponent(cloudName)}/video/upload`);
      xhr.upload.onprogress = (event) => {
        if (event.lengthComputable) setSuccess(`Uploading to Cloudinary... ${Math.round((event.loaded / event.total) * 100)}%`);
      };
      xhr.onload = () => {
        try {
    ...
    if (!response?.public_id || !response?.secure_url) throw new Error('Cloudinary did not return a usable video URL.');
    return response;
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (uploadMode === 'embed') {
        ...
        if (!isAdmin && (uploadMode === 'upload18' || uploadMode === 'uqload')) throw new Error(`Admin access required for ${uploadMode === 'upload18' ? 'Upload18' : 'UQLOAD'} uploads.`);
        if (!selectedFile) throw new Error('Please select a video file.');
        ...
        if (uploadMode === 'cloudinary') {
          ...
          setSuccess('Cloudinary upload completed successfully.');
  ...
  return (
    <div className="min-h-screen bg-black text-white p-6 md:p-10"><div className="max-w-3xl mx-auto space-y-6">
      <div><h1 className="text-3xl font-black">Upload Video</h1><p className="text-sm text-zinc-400 mt-1">Sign in is required for all video uploads. UQLOAD, Upload18, and Embed are admin-only.</p></div>
      <div className="flex bg-[#0a0a0a] border border-white/10 rounded-xl p-1">{(['bunny', 'uqload', 'upload18', 'cloudinary', 'embed'] as UploadMode[]).map((mode) => <button key={mode} type="button" onClick={() => setUploadMode(mode)} className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-colors ${uploadMode === mode ? 'bg-amber-500/20 text-amber-400' : 'text-zinc-400 hover:text-white'}`}>{mode === 'bunny' ? 'Bunny Stream' : mode === 'uqload' ? 'UQLOAD Stream' : mode === 'upload18' ? 'Upload18' : mode === 'cloudinary' ? 'Cloudinary' : 'Any Embed Link'}</button>)}</div>
