...
type UploadMode = 'bunny' | 'uqload' | 'upload18' | 'doodstream' | 'embed';
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (uploadMode === 'embed') {
        ...
        if (!isAdmin) throw new Error(`Admin access required for ${uploadMode === 'upload18' ? 'Upload18' : uploadMode === 'doodstream' ? 'DoodStream' : 'UQLOAD'} uploads.`);
        if (!selectedFile) throw new Error('Please select a video file.');
