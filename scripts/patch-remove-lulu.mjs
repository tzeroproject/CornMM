...
// Remove any remaining UI/menu lines that reference LuluStream.
source = source.split("\n").filter(line => !/lulu/i.test(line)).join("\n");
...
server = server.split("\n").filter(line => !/LULU_API_KEY|\/api\/lulu\/upload|handleLuluUpload/i.test(line)).join("\n");
if (/\/api\/lulu\/upload|handleLuluUpload|LULU_API_KEY/i.test(server)) {
